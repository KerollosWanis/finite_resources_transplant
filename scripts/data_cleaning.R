...
candidates_analysis <- 
  candidates_analysis %>% group_by(patient_id) %>% 
  filter(is.na(dplyr::first(MELD))==F,
         is.na(dplyr::first(MELD_exception))==F,
         is.na(dplyr::first(status1))==F,
         is.na(dplyr::first(gender))==F,
         is.na(dplyr::first(age))==F,
         is.na(dplyr::first(race))==F,
         is.na(dplyr::first(height))==F,
         is.na(dplyr::first(weight))==F,
         is.na(dplyr::first(accept_incompatible_blood_type))==F,
         is.na(dplyr::first(accept_extra_corporeal_liver))==F,
         is.na(dplyr::first(accept_liver_segment))==F,
         is.na(dplyr::first(accept_HBV_positive_donor))==F,
         is.na(dplyr::first(accept_HCV_positive_donor))==F,
         is.na(dplyr::first(patient_state))==F,
         is.na(dplyr::first(patient_on_life_support))==F,
         is.na(dplyr::first(functional_status))==F,
         is.na(dplyr::first(primary_diagnosis))==F,
         is.na(dplyr::first(spontaneous_bacterial_peritonitis))==F,
         is.na(dplyr::first(history_of_PV_thrombosis))==F,
         is.na(dplyr::first(history_of_TIPSS))==F
) %>% ungroup()
...
rm(stathist_liin, cand_liin, tx_li, donor_deceased, candidates, candidates_expanded)
gc()
